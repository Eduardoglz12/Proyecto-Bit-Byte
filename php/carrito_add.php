<?php
session_start();
//Incluir la conexión a la BDD para validar el stock
require 'db_conexion.php';

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['prod_id'])) {
    
    $prod_id = (int)$_POST['prod_id'];
    //Si se envía una cantidad (desde producto.php), úsala. Si no (desde index.php), usa 1.
    ...
    //Obtener la cantidad que ya está en el carrito para este producto
    $cantidad_en_carrito = isset($_SESSION['carrito'][$prod_id]) ? $_SESSION['carrito'][$prod_id] : 0;

    //Consultar el stock disponible en la BDD
    ...
    if ($resultado->num_rows > 0) {
        ...
        //Comprobar si la cantidad deseada (lo que hay + lo que se agrega) supera el stock
...
//Redirigir a la página del CARRITO para que el usuario vea el producto añadido
